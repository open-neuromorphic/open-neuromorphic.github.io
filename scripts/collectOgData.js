...
async function getContributorImagePath(authorName) {
    ...
    const slug = authorName.toLowerCase().replace(/\s+/g, '-').replace(/[.]/g, '');
...
async function collectData() {
  ...
  const globalHash = createHash(Buffer.concat([templateBuffer, eventTemplateBuffer, logoBuffer, backgroundBuffer]));
  ...
  for (const mdFile of markdownFiles) {
    ...
    try {
      ...
      const isUpcomingEvent = upcoming === true && ['workshops', 'student-talks', 'hacking-hours'].includes(contentType);
      ...
      if (isUpcomingEvent) {
          ...
          pageData.eventTime = `${start_time} - ${end_time} ${time_zone}`;
