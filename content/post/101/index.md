...
Most neuromorphic chips (IBM TrueNorth, Intel Loihi, Stanford’s Neurogrid, SpiNNaker, BrainScales, SynSense Speck) implement a form of a spiking mechanism, inspired by the incredibly energy-efficient mechanism of transmitting information through action potentials in the brain. The computation in the biological system happens locally in the neuron, with a neuron in cortex integrating information from some thousands of synapses. 
It's an open question to what degree of biological realism we should model biological neural networks. Dendritic computation? Multicompartment models? Different ion channels? When in the beginning neuromorphic was a lot about mimicking biological complexity, it seems like the field has gotten more pragmatic recently, focusing on simpler neuron models and enabling non-binary spikes.
Recurrence, architecure

## Training methods
Not only the neuron models themselves are bio-inspired, but also the way they learn. Inspired from neuroscience, early spiking neural networks were trained using Hebbian learning rules, where the strength of the connection between two neurons is reinforced if there is some form of local causality. This works for very shallow networks, but is hard to steer/control. The advent of deep learning has made it possible to train SNNs in a supervised fashion using backpropagation. Here we can use all the modern deep learning tools available and build on top of them. This has resulted in the most powerful SNNs to date. SNNs can be seen as a special case of RNNs with binary activations, internal states depending on the neuron model and long sequence lengths. That makes them very difficult to train, because we need to balance the trade-off between keeping activation in the network low, but also passing on enough information from layer to layer! If done right, the sparse network activation is the reason why neuromorphic systems can be so energy-efficient. 
As a single spike carries very low information value, multiple of them need to be integrated over time to make meaning of it, if we're interested in the average activation over time.  Feeding sequential inputs makes SNNs a subclass of RNNs, which have largely been abandonded since the rise of transformers. It remains to be seen if it will be possible to scale SNNs to similar sizes as ANNs.
